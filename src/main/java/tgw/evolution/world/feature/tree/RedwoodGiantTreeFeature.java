...
public class RedwoodGiantTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
    ...
    private static final BlockState LOG = EvolutionBlocks.LOG_REDWOOD.get().getDefaultState().with(BlockLog.TREE, true);
    ...
    protected boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos pos, MutableBoundingBox boundsIn) {
        ...
        if (pos.getY() >= 1 && pos.getY() + trunkHeight + 1 < 256) {
            ...
            if (!RedwoodGiantTreeFeature.placeTreeOfHeight(worldIn, pos, trunkHeight)) {
                return false;
            }
            ...
            for (int placingTrunks = 0; placingTrunks < trunkHeight + 15; ++placingTrunks) {
                ...
                if (placingTrunks < trunkHeight) {
                    this.placeTrunk(changedBlocks, worldIn, trunkPos, boundsIn);
                    this.placeTrunk(changedBlocks, worldIn, trunkPos.east(), boundsIn);
                    this.placeTrunk(changedBlocks, worldIn, trunkPos.east(2), boundsIn);
                    this.placeTrunk(changedBlocks, worldIn, trunkPos.south(), boundsIn);
                    this.placeTrunk(changedBlocks, worldIn, trunkPos.south(2), boundsIn);
                    this.placeTrunk(changedBlocks, worldIn, trunkPos.east().south(), boundsIn);
                    this.placeTrunk(changedBlocks, worldIn, trunkPos.east(2).south(), boundsIn);
                    this.placeTrunk(changedBlocks, worldIn, trunkPos.east().south(2), boundsIn);
                    this.placeTrunk(changedBlocks, worldIn, trunkPos.east(2).south(2), boundsIn);
                }
