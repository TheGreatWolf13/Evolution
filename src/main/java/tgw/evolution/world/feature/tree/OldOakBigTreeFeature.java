...
public class OldOakBigTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
    ...
    private static final BlockState LOG = EvolutionBlocks.LOG_OLD_OAK.get().getDefaultState().with(TREE, true);
    ...
    public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader world, Random rand, BlockPos position, MutableBoundingBox box) {
        ...
        if (posY >= 1 && posY + trunkHeight + 1 < 256) {
            ...
            boolean isSoil = BlockUtils.canSustainSapling(((IBlockReader) world).getBlockState(posDown), EvolutionBlocks.SAPLING_OLD_OAK.get());
            if (!isSoil) {
                return false;
            }
            if (!placeTreeOfHeight(world, position, trunkHeight)) {
                return false;
            }
            //the NW corner sapling grows first
            TreeUtils.setDirtAt(world, posDown);
            TreeUtils.setDirtAt(world, posDown.east());
            TreeUtils.setDirtAt(world, posDown.south());
            TreeUtils.setDirtAt(world, posDown.south().east());
            ...
            for (int placingTrunks = 0; placingTrunks < trunkHeight; ++placingTrunks) {
                ...
                BlockState stateTrunk = ((IBlockReader) world).getBlockState(trunkPos);
                if (stateTrunk.isAir((IBlockReader) world, trunkPos) || stateTrunk.isIn(BlockTags.LEAVES)) {
                    this.placeTrunk(changedBlocks, world, trunkPos, box);
                    this.placeTrunk(changedBlocks, world, trunkPos.east(), box);
                    this.placeTrunk(changedBlocks, world, trunkPos.south(), box);
                    this.placeTrunk(changedBlocks, world, trunkPos.east().south(), box);
                }
            }
            ...
            for (int xAddition = -2; xAddition <= 0; ++xAddition) {
                for (int zAddition = -2; zAddition <= 0; ++zAddition) {
                    int yAddition = -1;
                    this.placeLeaves(world, changedPosX + xAddition, changedPosY + yAddition, changedPosZ + zAddition);
                    this.placeLeaves(world, 1 + changedPosX - xAddition, changedPosY + yAddition, changedPosZ + zAddition);
                    this.placeLeaves(world, changedPosX + xAddition, changedPosY + yAddition, 1 + changedPosZ - zAddition);
                    this.placeLeaves(world, 1 + changedPosX - xAddition, changedPosY + yAddition, 1 + changedPosZ - zAddition);
            ...
            //top 2x2 leaves
            if (rand.nextBoolean()) {
                this.placeLeaves(world, changedPosX, changedPosY + 2, changedPosZ);
                this.placeLeaves(world, changedPosX + 1, changedPosY + 2, changedPosZ);
                this.placeLeaves(world, changedPosX + 1, changedPosY + 2, changedPosZ + 1);
                this.placeLeaves(world, changedPosX, changedPosY + 2, changedPosZ + 1);
            }
            //middle leaf layer
            for (int xMiddleAdd = -3; xMiddleAdd <= 4; ++xMiddleAdd) {
                for (int zMiddleAdd = -3; zMiddleAdd <= 4; ++zMiddleAdd) {
                    if ((xMiddleAdd != -3 || zMiddleAdd != -3) &&
                        ...
                        this.placeLeaves(world, changedPosX + xMiddleAdd, changedPosY, changedPosZ + zMiddleAdd);
                    }
                }
            }
            //making branches
                    ...
                    if ((branchXposAdd < 0 || branchXposAdd > 1 || branchZposAdd < 0 || branchZposAdd > 1) && rand.nextInt(3) <= 0) {
                        ...
                        for (int leafXAdd = -1; leafXAdd <= 1; ++leafXAdd) {
                            for (int leafZAdd = -1; leafZAdd <= 1; ++leafZAdd) {
                                this.placeLeaves(world, changedPosX + branchXposAdd + leafXAdd, changedPosY, changedPosZ + branchZposAdd + leafZAdd);
                            }
                        }
