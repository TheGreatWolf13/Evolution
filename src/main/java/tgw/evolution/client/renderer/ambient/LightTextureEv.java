...
public class LightTextureEv extends LightTexture {
    ...
    public void enableLightmap() {
        ...
        GlStateManager.translatef(8F, 8F, 8F);
    ...
    public void updateLightmap(float partialTicks) {
        if (this.needsUpdate) {
            ...
            if (world != null) {
                float skyBrightness = world.getSunBrightness(1F);
                ...
                else if (waterBrightness > 0F && this.mc.player.isPotionActive(Effects.CONDUIT_POWER)) {
                ...
                for (int i = 0; i < 16; ++i) {
                    for (int j = 0; j < 16; ++j) {
                        ...
                        if (this.gameRenderer.getBossColorModifier(partialTicks) > 0F) {
                            float f13 = this.gameRenderer.getBossColorModifier(partialTicks);
                            colorR = colorR * (1F - f13) + colorR * 0.7F * f13;
                            colorG = colorG * (1F - f13) + colorG * 0.6F * f13;
                            colorB = colorB * (1F - f13) + colorB * 0.6F * f13;
                        }
                        ...
                        colorB = COLORS[2];
                        // Forge: fix MC-58177
                        colorR = MathHelper.clamp(colorR, 0f, 1f);
                        colorG = MathHelper.clamp(colorG, 0f, 1f);
                        colorB = MathHelper.clamp(colorB, 0f, 1f);
                        if (nightVisionBrightness > 0F) {
                            float f17 = 1F / colorR;
                            if (f17 > 1F / colorG) {
                                f17 = 1F / colorG;
                            }
                            ...
                            colorR = colorR * (1F - nightVisionBrightness) + colorR * f17 * nightVisionBrightness;
                            colorG = colorG * (1F - nightVisionBrightness) + colorG * f17 * nightVisionBrightness;
                            colorB = colorB * (1F - nightVisionBrightness) + colorB * f17 * nightVisionBrightness;
                        }
                        if (colorR > 1F) {
                            colorR = 1F;
                        }
                        ...
                        f15 = 1F - f15 * f15 * f15 * f15;
                        f16 = 1F - f16 * f16 * f16 * f16;
                        colorR = colorR * (1F - gamma) + f14 * gamma;
                        colorG = colorG * (1F - gamma) + f15 * gamma;
                        colorB = colorB * (1F - gamma) + f16 * gamma;
                        ...
                        int l = (int) (colorR * 255F);
                        int i1 = (int) (colorG * 255F);
                        int j1 = (int) (colorB * 255F);
