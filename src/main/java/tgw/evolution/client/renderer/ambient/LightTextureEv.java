...
public class LightTextureEv extends LightTexture {
    ...
    public LightTextureEv(GameRenderer gameRenderer, Minecraft mc) {
        ...
        this.mc = mc;
        this.lightTexture = new DynamicTexture(256, 256, false);
                    ...
                    for (int y = 0; y < 16; ++y) {
                        for (int sl = 0; sl < 16; ++sl) {
                            for (int bl = 0; bl < 16; ++bl) {
                                ...
                                float bLRed = getLightBrightness(level, bl);
                                float bLGreen = getLightBrightness(level, x);
                                float bLBlue = getLightBrightness(level, y);
                                if (level.effects().forceBrightLightmap()) {
                                    ...
                                    bLRed += addend * skyRed;
                                    bLGreen += addend * skyGreen;
                                    bLBlue += addend * skyBlue;
                                ...
                                //Disabled gamma, as it messes up the colours
                                ...
                                int blue = (int) bLBlue;
                                this.lightPixels.setPixelRGBA(x * 16 + bl, y * 16 + sl, 0xff00_0000 | blue << 16 | green << 8 | red);
                            }
                        }
                    }
                }
