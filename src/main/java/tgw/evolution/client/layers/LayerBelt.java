...
public class LayerBelt extends RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
    ...
    @Override
    public void render(PoseStack matrices, MultiBufferSource buffer, int packetLight, AbstractClientPlayer player, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float headYaw, float headPitch) {
        ItemStack beltStack = EvolutionClient.BELT_ITEMS.getOrDefault(player.getId(), ItemStack.EMPTY);
        if (!beltStack.isEmpty()) {
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(player, beltStack, ItemTransforms.TransformType.NONE, player.getMainArm() == HumanoidArm.LEFT, matrices, buffer, player.level, packetLight, OverlayTexture.NO_OVERLAY, 0);
            matrices.popPose();
        }
    }
}
