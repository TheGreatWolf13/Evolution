...
public class LArrayQueue implements LQueue {
    ...
    public LArrayQueue(int capacity) {
        if (capacity < 0) {
    ...
    @Override
    public void enqueueFirst(long k) {
        if (this.start == 0) {
    ...
    @Override
    public void enqueueMany(LList list) {
        if (!this.isEmpty()) {
            int size = this.size();
    ...
    protected final void ensureCapacity(int capacity) {
        ...
        this.array = LongArrays.ensureCapacity(this.array, capacity, 0);
        this.length = this.array.length;
    }
