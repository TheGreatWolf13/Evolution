...
public class BlockPeat extends BlockMass implements IReplaceable {
    ...
    public static void checkFallable(World worldIn, BlockPos pos, BlockState state) {
        ...
        if (!worldIn.isAirBlock(posDown) &&
            worldIn.getBlockState(posDown) != EvolutionBlocks.PEAT.get().getDefaultState().with(LAYERS, 1) &&
            worldIn.getBlockState(posDown) != EvolutionBlocks.PEAT.get().getDefaultState().with(LAYERS, 2) &&
            worldIn.getBlockState(posDown) != EvolutionBlocks.PEAT.get().getDefaultState().with(LAYERS, 3)) {
    ...
    public int getMass(BlockState state) {
        return state.get(LAYERS) * super.getMass(state) / 4;
    }
