...
public class BlockFire extends Block implements IReplaceable {
    ...
    public static final SoundType FIRE = new SoundType(0.5f, 2.6f, SoundEvents.BLOCK_FIRE_EXTINGUISH, SoundEvents.BLOCK_WOOL_STEP, SoundEvents.BLOCK_WOOL_PLACE, SoundEvents.BLOCK_WOOL_HIT, SoundEvents.BLOCK_WOOL_FALL);
    private static final Map<Direction, BooleanProperty> FACING_TO_PROPERTY_MAP = SixWayBlock.FACING_TO_PROPERTY_MAP.entrySet().stream().filter(entry -> entry.getKey() != Direction.DOWN).collect(Util.toMapCollector());
    ...
    public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
        return this.isValidPosition(stateIn, worldIn, currentPos) && ChunkStorageCapability.contains(worldIn.getWorld().getChunkAt(currentPos), EnumStorage.OXYGEN, 1) ? this.getStateForPlacement(worldIn, currentPos).with(AGE, stateIn.get(AGE)) : Blocks.AIR.getDefaultState();
    }
