...
public abstract class MixinMultiVariant {
    ...
    public @Nullable BakedModel bake(ModelBakery bakery,
        ...
        if (size == 1) {
            return bakery.bake(variant0.getModelLocation(), variant0);
        }
        ...
        if (simple) {
            ...
            for (int i = 0; i < size; ++i) {
                ...
                BakedModel baked = bakery.bake(variant.getModelLocation(), variant);
                builder.add(baked);
            }
        ...
        for (int i = 0, len = variants.size(); i < len; i++) {
            Variant variant = variants.get(i);
            BakedModel bakedmodel = bakery.bake(variant.getModelLocation(), variant);
