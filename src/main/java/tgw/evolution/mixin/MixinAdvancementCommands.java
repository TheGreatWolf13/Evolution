...
import tgw.evolution.capabilities.player.CapabilityToast;
import tgw.evolution.patches.PatchServerPlayer;

import java.util.Collection;

@Mixin(AdvancementCommands.class)
public abstract class MixinAdvancementCommands {
    ...
    private static int perform(CommandSourceStack source,
        ...
        if (action == AdvancementCommands.Action.REVOKE) {
            for (ServerPlayer player : targets) {
                CapabilityToast toastData = ((PatchServerPlayer) player).getToastStats();
                toastData.reset();
            }
        }
        if (i == 0) {
