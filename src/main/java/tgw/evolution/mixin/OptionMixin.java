    ...
    static {
        BIOME_BLEND_RADIUS = new ProgressOption("options.biomeBlendRadius",
                                                ...
                                                (op, d) -> {
                                                    ...
                                                    ((IMinecraftPatch) Minecraft.getInstance()).lvlRenderer().allChanged();
                                                },
                                                (ops, op) -> {
        ...
        FOV = new ProgressOption("options.fov",
                                 ...
                                 (ops, d) -> {
                                     ops.fov = d;
                                     ((IMinecraftPatch) Minecraft.getInstance()).lvlRenderer().needsUpdate();
                                 },
                                 (ops, op) -> {
                                     double value = op.get(ops);
                                     if (value == 70) {
        ...
        GRAPHICS = CycleOption.create("options.graphics",
                                      ...
                                      (ops, op, status) -> {
                                          ...
                                          if (status == GraphicsStatus.FABULOUS && warnlistManager.willShowWarning()) {
                                              ...
                                              ((IMinecraftPatch) mc).lvlRenderer().allChanged();
                                          }
        ...
        RENDER_CLOUDS = CycleOption.create("options.renderClouds",
                                           ...
                                           (ops, op, status) -> {
                                               ...
                                               if (Minecraft.useShaderTransparency()) {
                                                   RenderTarget target = ((IMinecraftPatch) Minecraft.getInstance()).lvlRenderer().getCloudsTarget();
                                                   if (target != null) {
