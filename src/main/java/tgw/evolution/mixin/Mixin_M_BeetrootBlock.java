...
public abstract class Mixin_M_BeetrootBlock extends CropBlock {

    @Shadow @Final private static VoxelShape[] SHAPE_BY_AGE;

    public Mixin_M_BeetrootBlock(Properties properties) {
        super(properties);
    }
    ...
    public void randomTick_(BlockState state, ServerLevel level, int x, int y, int z, Random random) {
        if (random.nextInt(3) != 0) {
            super.randomTick_(state, level, x, y, z, random);
        }
    }
}
