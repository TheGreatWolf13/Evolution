...
public abstract class Mixin_M_BlastFurnaceBlock extends AbstractFurnaceBlock {

    public Mixin_M_BlastFurnaceBlock(Properties properties) {
        super(properties);
    }
    ...
    public void animateTick_(BlockState state, Level level, int x, int y, int z, RandomGenerator random) {
        if (state.getValue(LIT)) {
            ...
            if (random.nextDouble() < 0.1) {
                level.playLocalSound(px, y, pz, SoundEvents.BLASTFURNACE_FIRE_CRACKLE, SoundSource.BLOCKS, 1.0F, 1.0F, false);
            }
