...
import tgw.evolution.patches.PatchSheetedDecalTextureGenerator;

@Mixin(SheetedDecalTextureGenerator.class)
public abstract class MixinSheetedDecalTextureGenerator extends DefaultedVertexConsumer implements PatchSheetedDecalTextureGenerator {

    @Shadow @Final private Matrix4f cameraInversePose;
    @Mutable @Shadow @Final private VertexConsumer delegate;
    ...
    @Override
    public void set(VertexConsumer consumer, Matrix4f pose, Matrix3f normal) {
        this.delegate = consumer;
        this.cameraInversePose.load(pose);
        this.cameraInversePose.invert();
        this.normalInversePose.load(normal);
