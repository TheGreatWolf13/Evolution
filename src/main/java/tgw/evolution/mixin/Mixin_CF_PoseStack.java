...
public abstract class Mixin_CF_PoseStack implements PatchPoseStack, HR {

    @Unique private final OList<PoseStack.Pose> poses;
    @Shadow @Final @DeleteField private Deque<PoseStack.Pose> poseStack;
    @Unique private int size;

    @ModifyConstructor
    public Mixin_CF_PoseStack() {
        Matrix4f pose = new Matrix4f();
        pose.setIdentity();
        Matrix3f normal = new Matrix3f();
        normal.setIdentity();
        this.poses = new OArrayList<>();
        this.poses.add(new PoseStack.Pose(pose, normal));
        this.size = 1;
    }
