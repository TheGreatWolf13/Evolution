...
import tgw.evolution.patches.replace.net.minecraft.client.searchtree.ReloadableIdSearchTree;
import tgw.evolution.patches.replace.net.minecraft.client.searchtree.ReloadableSearchTree;
...
public abstract class Mixin_CF_Minecraft extends ReentrantBlockableEventLoop<Runnable> implements PatchMinecraft, WindowEventHandler {
    ...
    private void continueAttack(boolean leftClick) {
        if (!leftClick || EvolutionClient.shouldRenderSpecialAttack()) {
            this.missTime = 0;
        }
    ...
    private void handleKeybinds() {
        ...
        boolean notPausedNorAttacking = !this.multiplayerPause && !EvolutionClient.shouldRenderSpecialAttack();
                ...
                if (!this.multiplayerPause) {
                    ...
                    if (usedStack.getItem() instanceof ICancelableUse cancelable) {
                        if (cancelable.isCancelable(usedStack, this.player) && this.cancelUseCooldown == 0) {
                            EvolutionClient.resetCooldown(this.player.getUsedItemHand());
        ...
        //Not using item
                    ...
                    if (this.options.keyAttack.isDown()) {
                        if (!this.multiplayerPause) {
                            ...
                            if (chargeAttackType != null) {
                                if (!this.attackKeyPressed && EvolutionClient.getMainhandIndicatorPercentage(0.0f) >= 1.0f) {
                                    if (++this.attackKeyTicks >= autoAttackTime) {
                                        //Hold Attack
                                        if (this.missTime == 0) {
                                            EvolutionClient.startChargeAttack(chargeAttackType);
                        ...
                        if (!this.multiplayerPause) {
                            if (!this.attackKeyPressed) {
                                if (EvolutionClient.getMainhandIndicatorPercentage(0.0f) >= 1.0f) {
                                    if (this.attackKeyTicks >= minAttackTime) {
                                        ...
                                        //Short Attack
                                        if (EvolutionClient.checkHitResultBeforeShortAttack()) {
                                            EvolutionClient.startShortAttack(mainhandStack);
                                        }
                                        else {
                                            shouldContinueAttacking = this.startAttack();
                                        }
            ...
            isAttackKeyDown = this.options.keyAttack.isDown();
            if (EvolutionClient.getMainhandIndicatorPercentage(0.0f) < 1) {
    ...
    public void setScreen(@Nullable Screen screen) {
        ...
        EvolutionClient.onGuiOpen(screen);
        this.screen = screen;
        ...
        switch (this.hitResult.getType()) {
            case ENTITY: {
                ...
                if (this.gameMode.getPlayerMode() != GameType.SPECTATOR) {
                    if (!this.attackKeyPressed) {
                        EvolutionClient.startShortAttack(this.player.getMainHandItem());
                        swingHand = false;
                    }
                    else {
                        return true;
            ...
            case MISS: {
                ...
                if (!this.player.isCreative() && (mainHandItem.isEmpty() || mainHandItem.getItem() instanceof IMelee)) {
                    EvolutionClient.startShortAttack(mainHandItem);
                }
                swingHand = false;
                break;
            }
    ...
    private void startUseItem() {
                    ...
                    if (actResult.consumesAction()) {
                        if (actResult.shouldSwing()) {
                            ...
                            if ((this.useFlags & 2) != 0 && (this.repeatedUse = ItemUtils.getRepeatedUse(stack)) != ItemUtils.RepeatedUse.NEVER) {
                                this.useFlags |= 1;
                                Direction direction = EvolutionClient.getDirectionFromInput(this.player.getDirection(), this.player.input);
                                if (direction != null) {
        ...
        for (InteractionHand hand : ArrayHelper.HANDS_OFF_PRIORITY) {
            if (hand == InteractionHand.MAIN_HAND) {
                if (EvolutionClient.getMainhandIndicatorPercentage(0) < 1) {
                    return;
                }
            }
            else {
                if (EvolutionClient.getOffhandIndicatorPercentage(0) < 1) {
                    return;
                }
            }
            ItemStack stack = this.player.getItemInHand(hand);
    ...
    private boolean startUseItemSpecial() {
        ...
        if ((this.useFlags >> 2 & 0b111) == 0) {
            direction = EvolutionClient.getDirectionFromInput(this.player.getDirection(), this.player.input);
            if (direction != null) {
                this.useFlags |= (byte) (direction.ordinal() + 1 << 2);
            }
        }
    ...
    public void tick() {
        ...
        EvolutionClient.postClientTick();
        this.profiler.pop();
    }
